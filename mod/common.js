 ...
 * @team JC
 * @version 1.0.5
 * @description 平行绳饿了么common,使用之前需安装md5、child_process依赖
 * @admin false
 * @module true
 * @public true
 * @priority 9999
...
const _0x483607 = "http://h5st.hppro1.hpnu.cn/v1";
...
const User_Agent = _0x57ecf6[_0x533bff(0, _0x57ecf6["length"] - 1)];
...
async function invalidCookieNotify(_0xb58b84, _0x2abfe4) {
    ...
    if (!_0x522695["has"]("wxUid")) {
        ...
        await sendNotify("登录失效通知", "### 通知 \n 备注为：" + _0x2abfe4 + "的饿了么登录已失效，请打开饿了么app自动续期", {
...
async function addCK(userId, cookie) {
    const body = {
        "userId": userId,
        "cookie": cookie
    };
    try {
        const response = await fetch(`${_0x483607}/add`, {
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
            throw new Error(`Failed to add cookie. Status: ${response.status}`);
        }
...
async function runOne(_0x403dbf, _0xcaf64c) {
    ...
    //CK记录
    let userInfo = await getUserInfo(_0x403dbf);
    await wait(1);
    if (userInfo.user_id) {
        const userId = userInfo.user_id
        await addCK(userId, _0x403dbf);
    }
...
async function getCookies() {
    let ckList = [];
    try {
        const elmKeys = await new BncrDB('elmUser').keys();
        // 使用map和Promise.all来处理异步操作
        const accountsPromises = elmKeys.map(async function (item) {
            let accountsString = await new BncrDB('elmUser').get(item);
            if (accountsString && accountsString.hasOwnProperty('accounts') && Array.isArray(accountsString.accounts)) {
                return accountsString.accounts.map(account => account.elmck);
            }
            return []; // 如果没有找到符合条件的accounts，则返回空数组
        });
        ...
        // 合并所有accounts中的ck
        ckList = allAccounts.flat(); // 使用flat方法将二维数组展平为一维数组
    } catch (error) {
        // 异常处理
        // 假设sysMethod是一个包含startOutLogs方法的对象
        sysMethod.startOutLogs('配置初始化出错：', error);
        // 可以根据需要决定是否要退出程序
    }
    return ckList;
}
